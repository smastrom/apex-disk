...
2. **Find the window to re-check** — determine the range of commits that need re-verifying:
   - `git log -1 --format=%H -- reference/ AGENTS.md CLAUDE.md README.md .claude/rules/ .claude/commands/ .cursor/rules/ .cursor/commands/ .cursor/hooks.json .coderabbit.yaml` gives the last commit that touched any tracked doc. Use `<that-sha>..HEAD` as the default window.
...
4. **Docs + tests + file-top comment sweep** — apply the same sweep as `/sync` step 4. Read [`reference/index.md`](../reference/index.md) for the **Changed paths → re-verify** table and the file-top comment checklist in [`.claude/commands/sync.md`](sync.md).

   The only difference is input: `/force-sync` evaluates the entire commit **window** (`<range>`) as one unit; use the table to pick docs/tests most likely to drift, then skim remaining `.md` files for eligibility.

   Stage everything that drifted into the same recovery commit (or split if the drift covers unrelated areas).

5. **Commit** — if step 4 produced changes, commit them (imperative subject ≤70 chars; body only when the _why_ isn't obvious; include the agent `Co-Authored-By` trailer; never skip hooks). If nothing drifted, skip (no empty commit) but still run step 6.
