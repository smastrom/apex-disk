...
3. **Docs sweep** — for each group, open **every** file under `docs/` and decide whether any of them no longer match the change (per the AGENTS.md pre-commit rule). Update any that don't — stage the doc edits alongside the commit whose change they describe, not in a separate commit.
