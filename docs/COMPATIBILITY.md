...
|---------|-----------|-------------------|
| `overflow: overlay` (native macOS auto-hide scrollbars over content) | 14.0 (WebKit) | Non-WebKit / older WebKit falls back to the `overflow: auto` declared on the preceding line — classic always-visible scrollbars. |
...
The animation, popover, and focus-ring systems are intentionally implemented in Vue + JS + `@floating-ui/dom` rather than the corresponding newer-Safari APIs (`startViewTransition`, the Popover API, `:focus-visible`), so behavior is uniform across every supported Safari version. The only features that progressively enhance on newer macOS are visual polish that degrades silently: native overlay scrollbars (Safari 14+) and `color-mix()` with custom-property operands (Safari 16.2+).
